...
locals {
  ...
  external_public_private_ip_tertiary = [
    for private in local.bigip_map["external_subnet_ids"] :
    private["private_ip_tertiary"]
    if private["public_ip"] == true
  ]
...
resource "azurerm_public_ip" "tertiary_external_public_ip" {
  count               = var.tertiary_vip_disable ? 0 : length(local.external_public_subnet_id)
  name                = "${local.instance_prefix}-tertiary-pip-ext-${count.index}"
  location            = data.azurerm_resource_group.bigiprg.location
  resource_group_name = data.azurerm_resource_group.bigiprg.name
  domain_name_label   = format("%s-third-ext-%s", local.instance_prefix, count.index)
  allocation_method   = "Static"
  sku                 = "Standard"
  tags = merge(local.tags, {
    Name = format("%s-tertiary-pip-ext-%s", local.instance_prefix, count.index)
    }
  )
}
...
resource "azurerm_network_interface" "external_nic" {
  ...
  dynamic "ip_configuration" {
    for_each = var.tertiary_vip_disable ? [] : [0]
    content {
      name                          = "${local.instance_prefix}-tertiary-ext-ip-${count.index}"
      subnet_id                     = local.external_private_subnet_id[count.index]
      private_ip_address_allocation = (length(local.external_private_ip_tertiary[count.index]) > 0 ? "Static" : "Dynamic")
      private_ip_address            = (length(local.external_private_ip_tertiary[count.index]) > 0 ? local.external_private_ip_tertiary[count.index] : null)
    }
  }
...
resource "azurerm_network_interface" "external_public_nic" {
  ...
  dynamic "ip_configuration" {
    for_each = var.tertiary_vip_disable ? [] : [0]
    content {
      name                          = "${local.instance_prefix}-tertiary-ext-public-ip-${count.index}"
      subnet_id                     = local.external_public_subnet_id[count.index]
      private_ip_address_allocation = (length(local.external_public_private_ip_tertiary[count.index]) > 0 ? "Static" : "Dynamic")
      private_ip_address            = (length(local.external_public_private_ip_tertiary[count.index]) > 0 ? local.external_public_private_ip_tertiary[count.index] : null)
      public_ip_address_id          = azurerm_public_ip.tertiary_external_public_ip[count.index].id
    }
  }
  tags = merge(local.tags, var.externalnic_failover_tags, {
